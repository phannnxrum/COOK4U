...
import { handleGetAllChefs } from "../PickChef/chefService";
...
const FindChefPage = () => {
  ...
  useEffect(() => {
    const fetchChefs = async () => {
      try {
        const response = await handleGetAllChefs();

        if (response && response.data) {
          setChefsData(response.data);
        }
  ...
  const handleSearch = (e) => {
    if (e.key === "Enter" || e.type === "click") {
      if (searchQuery.trim()) {
        navigate(
          `/home/findachef?search=${encodeURIComponent(searchQuery.trim())}`
        );
      } else {
  ...
  const filteredChefs = chefsData.filter((chef) => {
    ...
    // Rating filter
    if (
      parseFloat(filters.minRating) > 0 &&
      chef.rating < parseFloat(filters.minRating)
    ) {
      return false;
    }
              ...
              {sortedChefs.length > 0 ? (
                ...
                <div className="col-span-2 text-center py-12">
                  <p className="text-gray-500 text-lg">
                    Không tìm thấy đầu bếp nào phù hợp
                  </p>
                </div>
              )}
            </div>
